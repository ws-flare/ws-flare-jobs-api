...
describe('Nodes', () => {
    ...
    it('should get a list of nodes in a job', async () => {
        await client.post('/nodes').send({jobId: 'node1', name: 'test-node-1'}).expect(200);
        await client.post('/nodes').send({jobId: 'node1', name: 'test-node-2'}).expect(200);
        await client.post('/nodes').send({jobId: 'node2', name: 'test-node-3'}).expect(200);
        await client.post('/nodes').send({jobId: 'node2', name: 'test-node-4'}).expect(200);
